...
collision_time = 0  # متغیر برای نگهداری زمان برخورد
collision_cooldown = 1000  # زمان مورد نیاز برای هر بار برخورد به مانع
...
def end_game():
    global player_score, game_speed, count
...
while True:
    ...
    if pygame.sprite.spritecollide(catosaur_group.sprite, obstacle_group, False):
        if collision_time == 0:  # اگر هنوز زمان برخورد را ذخیره نکرده باشیم
            if count <= 1:
                game_over = True
            else:
                count -= 1
            death_sfx.play()
            collision_time = pygame.time.get_ticks()  # زمان برخورد را ذخیره کن

    # اگر زمان برخورد ذخیره شده باشد و زمان انتظار آن تمام شده باشد، count را یک واحد افزایش بده
    if collision_time != 0 and pygame.time.get_ticks() - collision_time >= collision_cooldown:
        collision_time = 0  # زمان برخورد را مجدداً صفر کن
